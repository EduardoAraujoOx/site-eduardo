...
<p>
    ...
    IBS (Imposto sobre Bens e Serviços) no Brasil, ano a ano, entre 2029 e 2033</strong> &mdash;
    ...
    substitui &mdash; a soma nacional de ICMS e ISS; (ii) uma projeção de como esse bolo deve
...
<p>
    ...
    &mdash; a mesma proporção observada em 2025, por premissa metodológica explicada na Seção 4.
</p>
...
<p>
    ...
    DCA Anexo I-C com diferença inferior a 1%, checado estado a estado (ver metodologia de
    reconciliação do Estudo 08 desta série). Já a coleta de ISS nacional via RREO Anexo 3
    apresentou cobertura muito baixa (menos de 15% dos municípios respondem a essa conta
    específica) e foi descartada em favor do DCA, que atinge cobertura de 90% a 99% dos
    municípios brasileiros em cada ano.
</p>
...
<p>
    ...
    (&asymp;{fmtpct(meta['razao_bolo_pib_base'])} do PIB) a partir de 2026 &mdash; ou seja,
    elasticidade unitária do bolo tributário em relação ao PIB nominal. Essa não é uma escolha
    arbitrária de modelagem: é a mesma premissa de neutralidade de receita do art. 130 do ADCT
    (incluído pela EC 132/2023), segundo o qual as alíquotas do IBS devem ser calibradas para
    preservar a carga tributária agregada, e não para fazê-la crescer ou cair estruturalmente em
    relação à economia.
</p>
<p>
    Entre 2019 e 2025, a razão ICMS+ISS/PIB realizada variou entre
    {fmtpct(meta['faixa_historica_razao_2019_2025']['min'])} e
    {fmtpct(meta['faixa_historica_razao_2019_2025']['max'])} &mdash; uma banda de cerca de
...
<div class="law-box">
    <span class="art">ADCT art. 128</span> &mdash; A alíquota do ICMS e do ISS será reduzida a
    cada ano, de 2029 a 2032, à razão de um décimo, e extinta em 2033.<br>
    <span class="art">ADCT art. 131</span> &mdash; No mesmo período, o IBS é implementado
    progressivamente, complementando a fração já extinta do ICMS/ISS.
</div>
<p>
    O cronograma usado nesta nota técnica é idêntico ao adotado no Estudo 06 (Projeção ICMS/IBS
    por Estado, 2029&ndash;2033) desta mesma série de estudos:
</p>
<table>
...
<p>
    Diferentemente do Estudo 06, esta nota técnica não precisa dos coeficientes de redistribuição
    por unidade federativa (&phi;<sup>neutro</sup>, &phi;<sup>CPT</sup>, &phi;<sup>dest</sup>) nem
    da fração &alpha;<sub>a</sub> (critério histórico vs. destino): esses parâmetros determinam
    <em>como</em> o bolo é dividido entre estados e municípios, não o seu <em>tamanho total</em>.
    No agregado nacional, o valor total de IBS depende apenas de f<sub>a</sub>/s<sub>a</sub>.
</p>
...
<ol>
    ...
    &mdash; com apenas 11 observações anuais e quebras estruturais conhecidas (pandemia em 2020,
    ...
    parcela distribuída aos entes pelo critério destino &mdash; não sobre o total nacional
    arrecadado. Para a receita líquida que cada estado efetivamente recebe, com essas deduções,
    ver o Estudo 06 (Projeção ICMS/IBS por Estado).</li>
...
<ol class="refs">
    ...
    <li>BANCO CENTRAL DO BRASIL. <em>Sistema de Expectativas de Mercado &mdash; Boletim Focus</em>,
    ...
    <li>ARAÚJO, Eduardo Reis. <em>Projeção ICMS/IBS por Estado 2029&ndash;2033 (Estudo 06)</em> e
    <em>Receita-Base de referência de estados e municípios com IBS/ICMS entre 2029 e 2033
    (Estudo 09)</em>. Disponível em: www.eduardoreisaraujo.com.br/estudos/.</li>
</ol>
